...
class TriMesh {
  ...
  drawTriangles(program) {
    ...
    gl.vertexAttribPointer(
      program.locations.vertexPosition,
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, this.VertexNormalBuffer);
    gl.vertexAttribPointer(
      program.locations.vertexNormal,
  ...
  generateNormals() {
    ...
    for (var i = 0; i < this.numFaces; i++) {
      ...
      var v1Vec = glMatrix.vec3.fromValues(
        this.vBuffer[3 * v1],
        this.vBuffer[3 * v1 + 1],
        this.vBuffer[3 * v1 + 2]
      );
      var v2 = this.fBuffer[3 * i + 1];
      var v2Vec = glMatrix.vec3.fromValues(
        this.vBuffer[3 * v2],
        this.vBuffer[3 * v2 + 1],
        this.vBuffer[3 * v2 + 2]
      );
      var v3 = this.fBuffer[3 * i + 2];
      var v3Vec = glMatrix.vec3.fromValues(
        this.vBuffer[3 * v3],
        this.vBuffer[3 * v3 + 1],
        this.vBuffer[3 * v3 + 2]
      );
    ...
    for (var i = 0; i < this.numNormals; i++) {
      var n = glMatrix.vec3.fromValues(
        this.nBuffer[3 * i],
        this.nBuffer[3 * i + 1],
        this.nBuffer[3 * i + 2]
      );
